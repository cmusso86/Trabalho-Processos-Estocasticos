...
```{r echo=F, out.width="50%", fig.cap="Exemplo de cadeia de Markov em um exemplo de saúde. S pode ser um estado Saudável, I o estado doente e R, o estado absorvente da morte."}
...
Um processo semi-Markoviano é um processo é um processo a tempo contínuo. Dessa forma se considerar t interio, ou seja a cadeia mudar em uma unidade de tempo, t=1,2, o processo é de Markov a tempo discreto

Um processo estocástico $\{N(t): t \geq 0 \}$ que pode estar em qualquer
um de N estados $(1, 2, ..., N)$ e, a cada vez que entrar em um estado
**i**, lá permanecer por uma quantidade de tempo aleatória, com média
$\mu_i$ e, então, ir para um estado **j** com probabilidade $P_{ij}$ é
chamado de *processo semi-markov*. Diferencia-se de uma cadeia de Markov
por, nesta última, o tempo em que um processo passa em cada estado antes
de uma transição ser fixo.
...
em cada estado for  um valor fixo, o processo semi-Markov se torna
uma cadeia de Markov. A proporção de tempo em cada estado em um processo
...
modelos semi-Markov foram desenvolvidos. Eles permitem incorporar a
...
Os as probabilidades de transição foram estimados a partir da prevalência da doença na população, taxas de incidência e risco. 
...
Os parâmetros da cadeia de transição foram obtidos como descrito anteriormente, assim como o custo dos estados. Seguindo as intruções do pacote, temos: 
